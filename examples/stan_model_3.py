...
# THREE CODE
model = stan.build(stan_model, data=stan_input, random_seed=1)
fit = model.sample(num_samples=10000,
                   num_chains=5,
                   num_warmup=9500,
                   delta=0.99)
...
az.plot_trace(fit, var_names=['om', 'w'])
az.plot_autocorr(fit, var_names=['om', 'w'])
az.plott_kde(fit['om'], fit['w'])
